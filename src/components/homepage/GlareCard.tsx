...
export const GlareCard = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  ...
  return (
    ...
    >
      <div className='duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] grid h-full w-full origin-center overflow-hidden rounded-[var(--radius)] border border-slate-800/20 backdrop-blur-sm transition-transform will-change-transform [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] hover:filter-none hover:[--duration:200ms] hover:[--easing:linear] hover:[--opacity:0.6]'>
