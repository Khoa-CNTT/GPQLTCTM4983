...
export const modifiedTrackerTypeForComboBox = (type: any) => {
  return type?.map((item: any) => ({
    value: item.id,
    label: `${item.name}${item.currentAmount ? ` - ${formatCurrency(item.currentAmount, 'đ').split(',').join('.')}` : ''}`,
    ...item
  }))
}
...
export const initTrackerTypeData = (
  ...
  const [income, expense, transfer] = data.reduce(
    (
      ...
      tx: ITrackerTransactionType
    ) => {
      if (tx.type === ETypeOfTrackerTransactionType.INCOMING) {
        acc[0].push(tx)
      } else if (tx.type === ETypeOfTrackerTransactionType.EXPENSE) {
        acc[1].push(tx)
      } else {
        acc[2].push(tx)
      }
